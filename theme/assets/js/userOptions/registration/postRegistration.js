...
class PostRegistration{	
    ...
    registration(session){        
        $("#formPostRegistration").submit(function(event){				
            event.preventDefault();
            const formData = new FormData(document.querySelector('#formPostRegistration'));
            /*DEBUG
            ...
            */            
            ...
            if (appKey) {
                ...
                fetch(action+"/api/post/post_registration", {
                    method: 'POST',
                    body: formData
                })
                ...
                .then(function (response){
                    ...
                    if(response.data === "Postagem cadastrada com sucesso!"){
                        alert(response.data);
                        $("#formPostRegistration")[0].reset();
                        $("#smgErrorRegistrationPost").html("");
                    }else{
...
let startPostRegistration = new PostRegistration(new Session(false));